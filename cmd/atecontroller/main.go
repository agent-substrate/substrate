...
var (
	...
	ateapiAuthMode   = flag.String("ateapi-auth", "mtls", "Client auth to ateapi: mtls|jwt. 'mtls' (default) dials with insecure TLS and relies on pod-projected mTLS credentials for identity. 'jwt' verifies the server cert and sends a Bearer SA token.")
	ateapiCAFile     = flag.String("ateapi-ca-file", "", "PEM file with CAs trusted to verify the ateapi server cert. Required for jwt.")
	ateapiServerName = flag.String("ateapi-server-name", "", "SNI / hostname expected on the ateapi server cert. Optional.")
	ateapiTokenFile  = flag.String("ateapi-token-file", "", "Projected SA token file used as Bearer credential. Required for jwt.")
)
