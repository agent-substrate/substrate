...
func runShim(t *testing.T, env []string, args ...string) (invocations []string, exitCode int) {
	...
	cmd := exec.Command("bash", filepath.Join(root, "hack", "install-ate.sh"))
...
`
	...
	cmd.Dir = root
	cmd.Env = []string{"PATH=" + bin + ":" + os.Getenv("PATH"), "HOME=" + os.Getenv("HOME")}
...
func shimUsage(t *testing.T) string {
	t.Helper()
	root := repoRoot(t)
	cmd := exec.Command("bash", filepath.Join(root, "hack", "install-ate.sh"), "--help")
	cmd.Dir = root
