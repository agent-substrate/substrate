...
const (
	...
	// SystemInfoFsTag is the virtio-fs tag for the actor's system-info share,
	// served by a third virtiofsd. Contents are generated by atelet on the
	// host on every Run/Restore; containers see them read-only.
	SystemInfoFsTag = "ateSystemInfo"
	// guestSystemInfoDir is where the agent mounts SystemInfoFsTag in the
	// guest; each volume's contents live at <guestSystemInfoDir>/<volumeName>
	// and are bind-mounted read-only into the containers that declare the
	// volume.
	guestSystemInfoDir = "/run/ateom-system-info"
)
...
//
// withDurableShare additionally mounts the writable durable-dir share, and
// withSystemInfoShare the system-info share; the per-volume subdirectories of
// each are what the containers bind-mount at their declared paths.
func (a *AgentClient) CreateSandboxForActor(ctx context.Context, sandboxID, hostname string, withDurableShare, withSystemInfoShare bool) error {
	...
	if withSystemInfoShare {
		storages = append(storages, &agentpb.Storage{
			Driver:     virtioFSDriver,
			Source:     SystemInfoFsTag,
			Fstype:     typeVirtioFS,
			MountPoint: guestSystemInfoDir,
		})
	}
