...
func (g *gcsClient) PutSparseFile(ctx context.Context, bucket, object string, f *os.File) (res objectstorage.SparseUploadResult, err error) {
	size, populated, groups, err := objectstorage.PlanSparseFile(f)
	if err != nil {
		return res, err
	}
	res = objectstorage.SparseUploadResult{LogicalBytes: size, PopulatedBytes: populated}

	if len(groups) < 2 {
		// Too small to be worth splitting; the plain streaming path handles it.
		return res, objectstorage.ErrSparseTooSmall
	}
	...
	for i, ranges := range groups {
		grp.Go(func() error {
			...
			if err := objectstorage.WriteSparsePart(w, f, size, ranges, i == 0, i == len(groups)-1); err != nil {
				_ = w.Close()
