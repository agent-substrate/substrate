...
func TestNewAtomicWriter(t *testing.T) {
	targetDir, err := mkTmpdir("atomic-write")
	if err != nil {
	...
	nonExistentDir, err := mkTmpdir("atomic-write")
	if err != nil {
		t.Fatalf("unexpected error creating tmp dir: %v", err)
	}
...
func TestPathsToRemove(t *testing.T) {
	...
	for _, tc := range cases {
		...
		err = writer.Write(t.Context(), tc.payload1, nil)
		if err != nil {
		...
		actual, err := writer.pathsToRemove(t.Context(), tc.payload2, filepath.Join(targetDir, oldTsDir))
		if err != nil {
...
IAAAAAAAsDyZDwU=`
	...
	for _, tc := range cases {
		...
		writer := &AtomicWriter{targetDir: targetDir}
		err = writer.Write(t.Context(), tc.payload, nil)
...
func TestUpdate(t *testing.T) {
	...
	for _, tc := range cases {
		...
		err = writer.Write(t.Context(), tc.first, nil)
		if err != nil {
			t.Errorf("%v: unexpected error writing: %v", tc.name, err)
			continue
		}
		...
		err = writer.Write(t.Context(), tc.next, nil)
		if err != nil {
			if tc.shouldWrite {
				t.Errorf("%v: unexpected error writing: %v", tc.name, err)
				continue
...
func TestMultipleUpdates(t *testing.T) {
	...
	for _, tc := range cases {
		...
		for _, payload := range tc.payloads {
			writer.Write(t.Context(), payload, nil)
...
func TestSetPerms(t *testing.T) {
	...
	writer := &AtomicWriter{targetDir: targetDir}
	err = writer.Write(t.Context(), payload1, func(subPath string) error {
	...
	err = writer.Write(t.Context(), payload2, func(_ string) error {
