...
func (w *AtomicWriter) Write(ctx context.Context, payload map[string]FileProjection, setPerms func(subPath string) error) error {
	...
	if err != nil {
		return fmt.Errorf("while validating payload: %w", err)
	}
	...
	if err != nil {
		if !os.IsNotExist(err) {
			return fmt.Errorf("while reading link for data directory: %w", err)
		}
	...
	if len(oldTsDir) != 0 {
		// (3)
		pathsToRemove, err = w.pathsToRemove(ctx, cleanPayload, oldTsPath)
		if err != nil {
			return fmt.Errorf("while determining user-visible files to remove: %w", err)
		}

		// (4)
		if should, err := shouldWritePayload(cleanPayload, oldTsPath); err != nil {
			return fmt.Errorf("while determining whether payload should be written to disk: %w", err)
		} else if !should && len(pathsToRemove) == 0 {
			slog.InfoContext(ctx, "write not required for data directory", slog.String("dir", oldTsDir))
			...
			slog.InfoContext(ctx, "write required for target directory", slog.String("dir", w.targetDir))
		}
	}
	...
	if shouldWrite {
		...
		if err != nil {
			return fmt.Errorf("while creating new ts data directory: %w", err)
		}
		tsDirName := filepath.Base(tsDir)

		// (6)
		if err = w.writePayloadToDir(cleanPayload, tsDir); err != nil {
			return fmt.Errorf("while writing payload to ts data directory %s: %w", tsDir, err)
		}

		slog.InfoContext(ctx, "performed write of new data to ts data directory", slog.String("dir", tsDir))

		// (7)
		if setPerms != nil {
			if err := setPerms(tsDirName); err != nil {
				return fmt.Errorf("while applying ownership settings: %w", err)
			}
		}
		...
		if err = os.Symlink(tsDirName, newDataDirPath); err != nil {
			if err := os.RemoveAll(tsDir); err != nil {
				return fmt.Errorf("while removing new ts directory %s: %w", tsDir, err)
			}
		}
		...
		if runtime.GOOS == "windows" {
			if err := os.Remove(dataDirPath); err != nil {
				slog.ErrorContext(ctx, "Error removing data dir directory", slog.Any("err", err), slog.String("dir", dataDirPath))
			}
			err = os.Symlink(tsDirName, dataDirPath)
			if err := os.Remove(newDataDirPath); err != nil {
				slog.ErrorContext(ctx, "Error removing new data dir directory", slog.Any("err", err), slog.String("dir", newDataDirPath))
			}
		} else {
		...
		if err != nil {
			...
			if err := os.RemoveAll(tsDir); err != nil {
				slog.ErrorContext(ctx, "Error removing new ts directory", slog.Any("err", err), slog.String("dir", tsDir))
			}
			return fmt.Errorf("while renaming symbolic link for data directory: %s: %w", newDataDirPath, err)
		}
	}

	// (10)
	if err = w.createUserVisibleFiles(cleanPayload); err != nil {
		return fmt.Errorf("while creating visible symlinks in %s: %w", w.targetDir, err)
	}

	// (11)
	if err = w.removeUserVisiblePaths(ctx, pathsToRemove); err != nil {
		return fmt.Errorf("while removing old visible symlinks: %w", err)
	}

	// (12)
	if len(oldTsDir) > 0 {
		if err = os.RemoveAll(oldTsPath); err != nil {
			return fmt.Errorf("while removing old data directory %s: %w", oldTsDir, err)
		}
	}
...
func (w *AtomicWriter) pathsToRemove(ctx context.Context, payload map[string]FileProjection, oldTSDir string) (sets.Set[string], error) {
	...
	slog.DebugContext(ctx, "current paths", slog.String("targetDir", w.targetDir), slog.Any("paths", sets.List(paths)))
	...
	slog.DebugContext(ctx, "new paths", slog.String("targetDir", w.targetDir), slog.Any("paths", sets.List(newPaths)))

	result := paths.Difference(newPaths)
	slog.DebugContext(ctx, "paths to remove", slog.String("targetDir", w.targetDir), slog.Any("paths", result))
...
func (w *AtomicWriter) newTimestampDir() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("while creating new temp directory: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("while setting mode on new temp directory: %w", err)
	}
...
func (w *AtomicWriter) writePayloadToDir(payload map[string]FileProjection, dir string) error {
	for userVisiblePath, fileProjection := range payload {
		...
		if err := os.MkdirAll(baseDir, os.ModePerm); err != nil {
			return fmt.Errorf("while creating directory %s: %w", baseDir, err)
		}

		if err := os.WriteFile(fullPath, content, mode); err != nil {
			return fmt.Errorf("while writing file %s with mode %v: %w", fullPath, mode, err)
		}
		...
		if err := os.Chmod(fullPath, mode); err != nil {
			return fmt.Errorf("while changing file %s with mode %v: %w", fullPath, mode, err)
		}
		...
		if err := w.lchown(fullPath, int(*fileProjection.FsUser), -1); err != nil {
			return fmt.Errorf("while changing file %s to owner %v: %w", fullPath, int(*fileProjection.FsUser), err)
		}
	}
	...
	for userVisiblePath, fileProjection := range payload {
		...
		if err != nil && os.IsNotExist(err) {
			...
			if err := w.lchown(visibleFile, int(*fileProjection.FsUser), -1); err != nil {
				return fmt.Errorf("while changing file %s to owner %v: %w", visibleFile, int(*fileProjection.FsUser), err)
			}
		}
	}
	return nil
...
func (w *AtomicWriter) removeUserVisiblePaths(ctx context.Context, paths sets.Set[string]) error {
	...
	for p := range paths {
		...
		if err := os.Remove(filepath.Join(w.targetDir, p)); err != nil {
			slog.ErrorContext(ctx, "Error pruning old user-visible path", slog.String("path", p), slog.Any("err", err))
			lasterr = err
		}
	}
