...
// (st_blocks), not its logical length. The distinction matters here more
// than almost anywhere: the cache's biggest artifacts are sparse guest
// memory images, whose logical size can exceed their footprint by orders of
// magnitude, and every byte figure the store reports is compared against
// real disk usage (statfs watermarks, byte budgets) — logical sizes would
// trigger eviction that frees nothing and report targets met that were not.
