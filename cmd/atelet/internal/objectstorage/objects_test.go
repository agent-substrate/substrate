...
// memStore is an in-memory Store for round-trip tests.
...
// sendZstd takes the pipe (compress/upload overlap) path used for streaming stores
// instead of staging a seekable temp file.
type streamingMemStore struct{ *memStore }

func (s *streamingMemStore) SupportsStreamingWrites() {}

func TestCustomSchemeStore(t *testing.T) {
	ctx := context.Background()
	store := newMemStore()
	const objectURL = "blob+local://snapshots/actor/checkpoint.json"
	want := []byte("checkpoint")

	if err := Write(ctx, store, objectURL, want); err != nil {
		t.Fatalf("Write: %v", err)
	}
	got, err := Read(ctx, store, objectURL)
	if err != nil {
		t.Fatalf("Read: %v", err)
	}
...
func TestSparseUploadStreamingRoundTrip(t *testing.T) {
	...
	if err := WriteFileCompressed(ctx, store, gsURL, srcPath); err != nil {
	...
	if err := ReadFileCompressed(ctx, store, gsURL, dstPath); err != nil {
...
func TestPlainZstdBackwardCompatRoundTrip(t *testing.T) {
	...
	// Write is uncompressed; use sendZstd with a non-file reader to
