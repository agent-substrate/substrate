...
// Store reads and writes named objects within storage buckets.
...
// StreamingStore is a Store that accepts non-seekable readers in PutObject.
// Compressed uploads stream directly to implementations with this capability;
// other stores receive a seekable temporary file.
type StreamingStore interface {
	Store
	SupportsStreamingWrites()
}

// Read returns the complete contents of objectURL.
func Read(ctx context.Context, store Store, objectURL string) ([]byte, error) {
	ctx, span := tracer.Start(ctx, "objectStorageRead")
	defer span.End()

	bucket, object, err := parseObjectURL(objectURL)
	if err != nil {
		return nil, fmt.Errorf("%w: while parsing url: %w", ateerrors.ReasonInvalidObjectURL, err)
	}

	rc, err := store.GetObject(ctx, bucket, object)
	if err != nil {
...
// Open streams the object at objectURL; the caller must Close the returned reader.
// Unlike Read it does not buffer the whole object in memory.
func Open(ctx context.Context, store Store, objectURL string) (io.ReadCloser, error) {
	bucket, object, err := parseObjectURL(objectURL)
	if err != nil {
		return nil, fmt.Errorf("%w: while parsing url: %w", ateerrors.ReasonInvalidObjectURL, err)
	}
...
// Write uploads the given bytes (uncompressed) to objectURL. Intended for
// small objects such as the snapshot manifest.
func Write(ctx context.Context, store Store, objectURL string, content []byte) error {
	ctx, span := tracer.Start(ctx, "objectStorageWrite")
	defer span.End()
	...
	if err := store.PutObject(ctx, bucket, object, bytes.NewReader(content)); err != nil {
		return fmt.Errorf("while putting object bucket=%q object=%q: %w", bucket, object, err)
	}
	return nil
}

// WriteFileCompressed compresses localFilePath and writes it to objectURL.
func WriteFileCompressed(ctx context.Context, store Store, objectURL string, localFilePath string) (err error) {
	ctx, span := tracer.Start(ctx, "objectStorageWriteFileCompressed")
	defer span.End()
	...
	if err := sendZstd(ctx, store, objectURL, localFile); err != nil {
...
// SparseFileStore marks a backend that can upload a sparse file directly, splitting
...
type SparseFileStore interface {
	Store
	PutSparseFile(ctx context.Context, bucket, object string, f *os.File) (SparseUploadResult, error)
}

// SparseUploadResult reports the source sizes for a sparse upload.
type SparseUploadResult struct {
	LogicalBytes   int64
	PopulatedBytes int64
}

// ErrSparseTooSmall tells callers to use the normal streaming upload path.
...
// sendZstd zstd-compresses content and uploads it to objectURL.
//
...
func sendZstd(ctx context.Context, store Store, objectURL string, content io.Reader) error {
	bucket, object, err := parseObjectURL(objectURL)
	if err != nil {
		return fmt.Errorf("while parsing URL: %w", err)
	}
	tStart := time.Now()
	if sparseStore, ok := store.(SparseFileStore); ok {
		if f, isFile := content.(*os.File); isFile {
			res, err := sparseStore.PutSparseFile(ctx, bucket, object, f)
			switch {
			case err == nil:
	...
	if _, ok := store.(StreamingStore); ok {
		return sendStreamingZstd(ctx, store, bucket, object, content, tStart)
	}
	return sendBufferedZstd(ctx, store, bucket, object, content, tStart)
}
...
func sendBufferedZstd(ctx context.Context, client Store, bucket, object string, content io.Reader, tStart time.Time) error {
...
func sendStreamingZstd(ctx context.Context, client Store, bucket, object string, content io.Reader, tStart time.Time) error {
	type result struct {
...
// ReadFileCompressed reads objectURL, decompresses it, and writes localFilePath.
func ReadFileCompressed(ctx context.Context, store Store, objectURL string, localFilePath string) (err error) {
	ctx, span := tracer.Start(ctx, "objectStorageReadFileCompressed")
	defer span.End()
	...
	if err := readCompressed(ctx, store, objectURL, localFile); err != nil {
		return fmt.Errorf("while fetching %q from object storage: %w", objectURL, err)
	}

	return nil
}

func readCompressed(ctx context.Context, store Store, objectURL string, out io.Writer) (err error) {
	bucket, object, err := parseObjectURL(objectURL)
	if err != nil {
		return fmt.Errorf("%w:while parsing URL: %w", ateerrors.ReasonInvalidObjectURL, err)
	}
...
func parseObjectURL(objectURL string) (string, string, error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("while parsing %q: %w", objectURL, err)
	}
