...
// SparseExtent is a populated byte range of a sparse source file.
type SparseExtent struct{ off, length int64 }

type extentRange = SparseExtent
...
// PlanSparseFile divides a sparse file's populated extents into upload parts.
func PlanSparseFile(src *os.File) (size, populated int64, groups [][]SparseExtent, err error) {
	fileInfo, err := src.Stat()
	if err != nil {
		return 0, 0, nil, err
	}
	size = fileInfo.Size()
	extents, populated, err := sparseExtents(src, size)
	if err != nil {
		return 0, 0, nil, err
	}
	return size, populated, planSparseParts(extents, populated, sparsePartCount(populated)), nil
}
...
// WriteSparsePart writes one planned part of the sparse-extent stream.
func WriteSparsePart(dst io.Writer, src *os.File, size int64, ranges []SparseExtent, first, last bool) error {
	return writeSparsePart(dst, src, size, ranges, first, last)
}
