...
//
// A checkpoint reads the actor's checkpoint dir for as long as it is
// uploading, which can be minutes for a large snapshot. Anything that clears
// that dir underneath it destroys the source mid-read, and the checkpoint then
// fails partway through and crashes an actor that was fine.
//
// Every RPC that clears an actor's on-node state takes it, not only Checkpoint:
// UploadPausedCheckpoint prunes every local snapshot, and Run, Restore and
// Terminate reset the actor's directories. Two checkpoints can also overlap on
// their own, when a lease expires and a new leader retries while the original
// handler's connection is still alive. A lock one caller can walk around is
// not a lock.
//
