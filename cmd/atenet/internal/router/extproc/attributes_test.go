...
func TestAttributeKeys(t *testing.T) {
	tests := []struct {
		...
		{"target actor filter state", TargetActorFilterStateKey, "dev.ate.target.actor"},
		{"target actor attribute", TargetActorFilterStateAttribute, "filter_state['dev.ate.target.actor']"},
