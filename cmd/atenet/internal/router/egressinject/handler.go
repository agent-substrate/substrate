...
// relayed here as the dev.ate.actor.identity filter-state attribute — never from
// a request header.
...
// forwards, carrying the actor's verified identity URI.
const actorIdentityAttribute = extproc.ActorIdentityFilterStateAttribute
...
func (h *Handler) HandleRequestHeaders(ctx context.Context, md *extproc.RequestMetadata) (extproc.Result, error) {
	...
	if err != nil {
		...
		// Fail open: if the policy cannot be fetched we inject nothing and let the
		// request proceed rather than block it. The credential is additive, so a
		// missing injection surfaces as an auth failure at the origin, not a
		// broken egress path.
		slog.ErrorContext(ctx, "egress-inject: fetching egress policy failed, passing through",
