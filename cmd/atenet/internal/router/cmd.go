...
func NewRouterCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&cfg.CredentialProvider.Name, "credential-provider-name", "", "Credential provider this egress gateway serves, as a substrate-secret:// prefix (e.g. substrate-secret://kubernetes.io); a policy credential URI naming any other provider is refused. Empty disables the check (dev only)")
