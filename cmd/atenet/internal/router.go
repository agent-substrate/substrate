...
func NewRouterCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&cfg.UpstreamSpiffePrefix, "upstream-spiffe-prefix", "spiffe://cluster.local/", "SPIFFE URI SAN prefix (trust domain) the actor's atunnel server cert must match. Empty falls back to Envoy's default SAN check against the dialed pod IP (which SPIFFE-only certs never match).")
