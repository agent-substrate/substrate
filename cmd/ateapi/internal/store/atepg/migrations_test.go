...
func TestMigrationsConcurrentStartup(t *testing.T) {
	...
	for range 2 {
		go func() {
			p, err := Connect(ctx, containerDSN, containerDSN, "", "", "concurrent-startup", 0)
			if p != nil {
				p.Close()
				p.pool.Close()
			}
...
func TestMigrationSchemaStates(t *testing.T) {
	...
	t.Run("ahead", func(t *testing.T) {
		...
		p, err := Connect(ctx, containerDSN, containerDSN, "", "", "migration-ahead", 0)
		if err != nil {
	...
	t.Run("tables without metadata", func(t *testing.T) {
		...
		_, err := Connect(ctx, containerDSN, containerDSN, "", "", "migration-legacy", 0)
