...
func seedActor(t *testing.T, ctx context.Context, st store.Interface, f actorFixture) {
	...
	actor := &ateapipb.Actor{
		...
		Status:                 &ateapipb.ActorStatus{State: f.status},
...
func runningOnNode(nodeName string) actorFixture {
	return actorFixture{status: ateapipb.ActorState_ACTOR_STATE_RUNNING, workerNode: nodeName}
}
...
func TestMintCertAuthorization(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"actor does not exist": {
			fixture: actorFixture{
				status:     ateapipb.ActorState_ACTOR_STATE_RUNNING,
				workerNode: testNode,
...
//
// STATE_RESUMING is the case that matters in practice. atelet mints while
...
func TestMintCertActorStatus(t *testing.T) {
	for value, name := range ateapipb.ActorState_name {
		actorStatus := ateapipb.ActorState(value)
		wantCode := codes.OK
		if actorStatus == ateapipb.ActorState_ACTOR_STATE_DELETING {
			wantCode = codes.FailedPrecondition
		}
		t.Run(name, func(t *testing.T) {
			ctx := context.Background()
...
func TestMintCertDeniesUnassignedActorWhateverItsStatus(t *testing.T) {
	for name, actorStatus := range map[string]ateapipb.ActorState{
