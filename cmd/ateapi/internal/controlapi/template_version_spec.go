...
// defaultActorTemplateVersion materializes server-applied defaults into the
// version before validation and storage, mirroring the CRD's API-server
...
func defaultActorTemplateVersion(version *ateapipb.ActorTemplateVersion) {
	for _, c := range version.GetContainers() {
		readyz := c.GetReadyz()
		if readyz == nil {
			continue
		}
...
// validateActorTemplateVersion ports the ActorTemplate CRD's CEL and
// kubebuilder invariants onto the proto version's workload definition. Call
// defaultActorTemplateVersion first: readyz fields are validated at their
// effective (defaulted) values.
func validateActorTemplateVersion(version *ateapipb.ActorTemplateVersion, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}

	microvm := version.GetSandboxConfig().GetSandboxClass() == ateapipb.SandboxClass_SANDBOX_CLASS_MICROVM

	if selector := version.GetWorkerSelector(); selector != nil {
		allErrs = append(allErrs, validateSelector(selector, fldPath.Child("worker_selector"))...)
	}

	volumeNames, volumeErrs := validateVolumes(version.GetVolumes(), microvm, fldPath.Child("volumes"))
	allErrs = append(allErrs, volumeErrs...)

	allErrs = append(allErrs, validateContainers(version.GetContainers(), volumeNames, fldPath.Child("containers"))...)

	// Every declared volume must be mounted by at least one container.
	mounted := mountedVolumeNames(version.GetContainers(), volumeNames)
	for i, v := range version.GetVolumes() {
	...
	allErrs = append(allErrs, validateSnapshotsConfig(version.GetSnapshotsConfig(), microvm, fldPath.Child("snapshots_config"))...)
	...
	if cfg := version.GetSandboxConfig(); cfg == nil {
		...
		pauseImagePath := cfgPath.Child("pause_image")
		if cfg.GetPauseImage() == "" {
			allErrs = append(allErrs, field.Required(pauseImagePath, ""))
		} else if !strings.Contains(cfg.GetPauseImage(), "@") {
			allErrs = append(allErrs, field.Invalid(pauseImagePath, cfg.GetPauseImage(), pinnedImageMsg))
		}
	}
...
func validateVolumeMounts(mounts []*ateapipb.VolumeMount, volumeNames map[string]bool, fldPath *field.Path) field.ErrorList {
	...
	for i, vm := range mounts {
		...
		if name := vm.GetName(); name == "" {
			...
			allErrs = append(allErrs, field.Invalid(namePath, name, "must match the name of a volume in volumes"))
		}
