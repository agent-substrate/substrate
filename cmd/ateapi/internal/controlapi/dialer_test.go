...
func TestAteletDialerClosesEvictedConn(t *testing.T) {
	ateletIndexer := newTestAteletIndexer(t,
		&corev1.Pod{
			ObjectMeta: metav1.ObjectMeta{Namespace: ateletNamespace, Name: "atelet-1", UID: types.UID("atelet-uid-1")},
			Spec:       corev1.PodSpec{NodeName: "node-1"},
			Status:     corev1.PodStatus{PodIPs: []corev1.PodIP{{IP: "10.0.0.1"}}},
		},
		&corev1.Pod{
			ObjectMeta: metav1.ObjectMeta{Namespace: ateletNamespace, Name: "atelet-2", UID: types.UID("atelet-uid-2")},
			Spec:       corev1.PodSpec{NodeName: "node-2"},
			Status:     corev1.PodStatus{PodIPs: []corev1.PodIP{{IP: "10.0.0.2"}}},
		},
	)

	d := NewAteletDialer(nil, ateletIndexer, "", "")
	d.ateletConns = lru.NewWithEvictionFunc(1, onEvict)
	d.dialCredentials = func(string) (credentials.TransportCredentials, error) {
		return insecure.NewCredentials(), nil
	}

	firstConn, err := d.DialForAteletOnNode("node-1")
	if err != nil {
		t.Fatalf("DialForAteletOnNode(node-1): %v", err)
	}

	secondConn, err := d.DialForAteletOnNode("node-2")
	if err != nil {
		t.Fatalf("DialForAteletOnNode(node-2): %v", err)
	}
