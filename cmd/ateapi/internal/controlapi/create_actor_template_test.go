...
func TestValidateCreateActorTemplateRequest(t *testing.T) {
	tests := []struct {
		...
		&ateapipb.CreateActorTemplateRequest{ActorTemplate: &ateapipb.ActorTemplate{
			Metadata: &ateapipb.ResourceMetadata{Atespace: "NS_1", Name: "tmpl-a"},
		}},
		...
		&ateapipb.CreateActorTemplateRequest{ActorTemplate: &ateapipb.ActorTemplate{
			Metadata: &ateapipb.ResourceMetadata{Atespace: "ns1", Name: "Tmpl_A"},
		}},
		...
		field.ErrorList{field.Forbidden(field.NewPath("actor_template", "default_version_on_create"), "")},
	}}
