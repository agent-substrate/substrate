...
func Validate_Actor(
	...
	{ // field ateapipb.Actor.Status
		fn := func(
			...
			// call field-attached validations
			earlyReturn := false
			if e := validate.IfOption(ctx, op, fldPath, obj, oldObj, "validateOutput", false, validate.ForbiddenValue).MarkShortCircuit(); len(e) != 0 {
				errs = append(errs, e...)
				earlyReturn = true
			}
	...
	// field ateapipb.Actor.WorkerAssignment has no validation
	// field ateapipb.Actor.InProgressSnapshotName has no validation
	...
	// field ateapipb.Actor.SourceSnapshot has no validation
	return errs
}
