...
func (s *Service) DeleteActorTemplateVersion(ctx context.Context, req *ateapipb.DeleteActorTemplateVersionRequest) (*ateapipb.ActorTemplateVersion, error) {
	...
	parentRef := resources.ActorTemplateRefFromObjectRef(version.GetActorTemplate())
	...
	deleted, err := s.persistence.DeleteActorTemplateVersion(lock.Context(), versionRef)
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			return nil, status.Errorf(codes.NotFound, "ActorTemplateVersion %s not found", versionRef)
		}
		if errors.Is(err, store.ErrFailedPrecondition) {
			return nil, status.Errorf(codes.FailedPrecondition, "ActorTemplateVersion %s is the default_version_on_create of ActorTemplate %s", versionRef, parentRef)
		}
