...
func withCapacity(actors int32) func(*ateapipb.Worker) {
	return func(w *ateapipb.Worker) {
		workerStatus(w).Capacity = &ateapipb.WorkerResources{Actors: actors}
	}
}
...
func TestRecycleWorkerWorkflow_ReleasesBoundActorAndKeepsWorker(t *testing.T) {
	...
	actor := seedAPIActor(t, ctx, persistence, ateapipb.ActorState_ACTOR_STATE_RUNNING, func(a *ateapipb.Actor) {
		a.Status.ExternalSnapshot = &ateapipb.ExternalSnapshot{SnapshotUri: "gs://bucket/last"}
		a.Status.LocalSnapshotInfo = &ateapipb.LocalSnapshotInfo{
			SnapshotName:              "local-1",
	...
	if c := got.GetStatus().GetCapacity().GetActors(); c != 4 {
		t.Errorf("capacity actors = %d, want 4 preserved; an ateom reports capacity once per process", c)
	}
	if a := got.GetStatus().GetAllocated().GetActors(); a != 0 {
	...
	// Crashed is not the same as unrecoverable, and a richer model of "bad but
	// not totally lost" is expected to land on top of this. Whatever it ends up
	// looking like, it can only work if the release does not destroy the state
	// that might still be salvageable, so pin that here: the completed durable
	// snapshot and the node-local one survive, and only the checkpoints that
	// were still being written go.
	if got := storedActor.GetStatus().GetExternalSnapshot().GetSnapshotUri(); got != "gs://bucket/last" {
		t.Errorf("external snapshot = %q, want it preserved", got)
	}
	if got := storedActor.GetStatus().GetLocalSnapshotInfo().GetSnapshotName(); got != "local-1" {
		t.Errorf("local snapshot = %q, want it preserved: the node is still there, unlike a deleted pod", got)
	}
	if storedActor.GetStatus().GetInProgressSnapshotName() != "" || storedActor.GetStatus().GetInProgressLocalSnapshotName() != "" {
		t.Errorf("in-progress checkpoints not cleared: %v", storedActor.GetStatus())
	}
}
