...
func (s *Service) ListActorTemplateVersions(ctx context.Context, req *ateapipb.ListActorTemplateVersionsRequest) (*ateapipb.ListActorTemplateVersionsResponse, error) {
	...
	versions, nextToken, err := s.persistence.ListActorTemplateVersions(ctx, req.GetAtespace(), resources.ActorTemplateRefFromObjectRef(req.GetActorTemplate()), effectivePageSize(req.GetPageSize()), req.GetPageToken())
	if err != nil {
...
func validateListActorTemplateVersionsRequest(req *ateapipb.ListActorTemplateVersionsRequest) field.ErrorList {
	...
	// An unset ref means "across all templates"; a set filter must be a valid
	// Atespaced reference.
