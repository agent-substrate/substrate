...
#
# Usage:
#   ./load.sh [-d duration_secs] [-r router_url] [actor_id ...]
#
# Examples:
...
Usage: ./load.sh [-d duration_secs] [-r router_url] [actor_id ...]
...
# Preflight: make sure the actors exist (idempotent; ignore "already exists").
echo "==> ensuring actors exist (template ${TEMPLATE})"
for a in "${ACTORS[@]}"; do
  kubectl ate create actor "${a}" --template "${TEMPLATE}" >/dev/null 2>&1 || true
done

# One worker per actor: hammer it with request->suspend until the deadline.
worker() {
  local actor="$1" host="$1.${SUFFIX}" log="${TMP}/$1.log"
  ...
  while [[ $(date +%s) -lt ${deadline} ]]; do
    ...
    kubectl ate suspend actor "${actor}" >/dev/null 2>&1 || true
  done
}
