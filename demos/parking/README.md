...
- Create a **2-replica** `WorkerPool` (`parking-pool`) and the `parking` `ActorTemplate`.
...
for id in p1 p2 p3 p4; do
  kubectl ate create actor "$id" --template ate-demo-parking/parking
done
```
...
```bash
curl -s -H "Host: p1.actors.resources.substrate.ate.dev" http://localhost:8000
curl -s -H "Host: p2.actors.resources.substrate.ate.dev" http://localhost:8000
...
curl -s -w '\n-> HTTP %{http_code} in %{time_total}s\n' \
  -H "Host: p3.actors.resources.substrate.ate.dev" http://localhost:8000
```
...
Delete the actors first — namespace teardown does not reclaim actor records or
their GCS snapshots:

```bash
for id in p1 p2 p3 p4; do kubectl ate delete actor "$id"; done
```

Then remove the template, pool, and namespace:
