...
var app = new Hono2();
var DEMO_NAMESPACE = process.env.DEMO_NAMESPACE || "openclaw";
var predefinedTasks = [
...
var taskCursor = 0;
var AGENT_META = {
  "Claw-Luna": { color: "#79c0ff", emoji: "\u{1F7E6}", id: "agent-luna" },
  "Claw-Mars": { color: "#ff79c6", emoji: "\u{1F7EA}", id: "agent-mars" },
  "Claw-Nova": { color: "#f1fa8c", emoji: "\u{1F7E8}", id: "agent-nova" }
};
...
var brokerRegistry = {
  "agent-luna": { agentId: "agent-luna", intervalSec: 45, lastWakeup: 0, nextWakeup: Date.now() / 1e3 + 45, enabled: false },
  "agent-mars": { agentId: "agent-mars", intervalSec: 90, lastWakeup: 0, nextWakeup: Date.now() / 1e3 + 90, enabled: false },
  "agent-nova": { agentId: "agent-nova", intervalSec: 120, lastWakeup: 0, nextWakeup: Date.now() / 1e3 + 120, enabled: false }
};
var stats = {
...
async function syncState() {
  try {
    ...
    clusterState.actors = actors.filter((a) => (a.actorId || a.actor_id).startsWith("agent-")).map((a) => {
    ...
    clusterState.pods = podsRaw.map((p) => {
      const activeActor = actors.find((a) => (a.ateomPodName || a.ateom_pod_name) === p.metadata.name);
    ...
    stats.totalLogicalActiveSec += runningActors * 0.4;
...
async function schedulerLoop() {
  ...
  for (const a of activeAssignments) {
    ...
    const actor = clusterState.actors.find((act) => act.name === actorMeta.id);
    if (!actor) continue;
    ...
    if (a.state === "running" && a.started_at && now - a.started_at > a.durationSec) {
      ...
      if (!moreTasksForMe) {
        ...
        enqueueLifecycleCmd(`kubectl-ate suspend actor ${actor.name}`).finally(() => {
          agentLocks[actor.name] = false;
        });
      }
      continue;
    }
    if (a.state === "queued" && status === "STATUS_SUSPENDED") {
      agentLocks[actor.name] = true;
      enqueueLifecycleCmd(`kubectl-ate resume actor ${actor.name}`).catch(async (e) => {
        if (e.message.includes("no free workers available")) {
          const contender = clusterState.actors.find((act) => {
          ...
          if (contender) {
            ...
            await enqueueLifecycleCmd(`kubectl-ate suspend actor ${contender.name}`).finally(() => {
              agentLocks[contender.name] = false;
            });
          }
        }
      }).finally(() => {
...
var port = process.env.PORT ? parseInt(process.env.PORT) : 8090;
