...
// demos/sub-agent-multiplex/workload/local-nano.ts
var LocalNanoProxy = class {
  actorId;
  brokerUrl;
  localQueue = [];
  constructor() {
  ...
  handleExternalTrigger(task, sender, source) {
...
[REHYDRATION] State successfully thawed on Substrate.`;
  ...
  async handleSendMessage(to, text) {
...
var local_nano_default = {
  port: 8080,
  fetch: app.fetch
};

// demos/sub-agent-multiplex/workload/agent-app.ts
var AgentApp = class {
  taskCounter = 0;
  localNanoUrl = "http://localhost:8080";
  constructor() {
    console.log("[AgentApp] Starting...");
    this.registerWithLocalNano();
    this.pollForTasks();
  }
  async registerWithLocalNano() {
    try {
...
app2.get("/health", (c) => c.json(agent.getStatus()));
var agent_app_default = {
  port: 8081,
  fetch: app2.fetch
};

// demos/sub-agent-multiplex/workload/index.ts
console.log("=== NanoClaw Fleet Management Master Initialized ===");
serve({
  fetch: local_nano_default.fetch,
  port: local_nano_default.port
}, (info) => {
  console.log(`[Infra] Local Nano Proxy active on port ${info.port}`);
});
serve({
  fetch: agent_app_default.fetch,
  port: agent_app_default.port
}, (info) => {
