...
const AGENT_META: Record<string, { color: string, id: string, interval: number }> = {
  "agent-luna": { color: "#79c0ff", id: "agent-luna-v12", interval: 60000 },
  ...
  try {
    ...
    try {
      ...
      brokerOut = await res.json();
       ...
       console.error("Broker Sync Error:", e.message);
    }
...
app.post("/api/deep-clean", async (c) => {
  try {
    // 1. Wipe all logical actors
    await runCmd(`kubectl-ate --endpoint ${ATE_ENDPOINT} delete actor agent-luna-v12`).catch(() => {});
    ...
    // 2. Restart physical workers
    await runCmd(`kubectl delete pods -n ${NS} -l app=agent-pool`);
    
    // 3. Recreate agents with fresh identities
    setTimeout(async () => {
       await runCmd(`kubectl-ate --endpoint ${ATE_ENDPOINT} create actor agent-luna-v12 --template sub-agent/sub-agent-agent`).catch(() => {});
...
<body>
<header>
  <h1>Fleet Management Master <span style="font-size:0.6em; vertical-align:middle; opacity:0.8;">V1.1.4 WHATSAPP</span></h1>
...
<div class="grid-master">
  ...
  <div class="card">
    ...
    <div class="card" style="margin-top: 15px; background: transparent; border:none; padding:0;">
      <h2 style="border-left-color: var(--yellow)">Substrate Economics</h2>
      <div class="metric-highlight-grid">
         <div class="metric-item"><div id="stat-density" class="metric-val">1.50x</div><div style="font-size:0.6em;color:var(--muted)">DENSITY</div></div>
         <div class="metric-item"><div id="stat-savings" class="metric-val">33.3%</div><div style="font-size:0.6em;color:var(--muted)">HW SAVINGS</div></div>
      </div>
    </div>
    <div class="card" style="margin-top: 15px; background: transparent; border:none; padding:0;">
...
<div class="grid-side">
  <div class="card"><h2>Task Timeline</h2><div id="timeline" class="timeline"></div></div>
  <div class="card"><h2>Physical Resource Map</h2><div id="pods"></div></div>
</div>

<div class="card" style="margin-bottom: 1.5em;"><h2>Logical Actor Fleet</h2><div id="actors" style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:15px;"></div></div>
...
async function trigger(agentKey) { await fetch("/api/give-task?agent=" + agentKey, { method: "POST" }); }
async function deepClean() { if(confirm("Initiate Hard Hardware Reset? This will wipe all actors and workers.")) { await fetch("/api/deep-clean", { method: "POST" }); } }
...
async function refresh() {
  try {
    ...
    document.getElementById("wa-status").innerHTML = '<span class="badge ' + data.connectionStatus + '">Status: ' + data.connectionStatus + '</span>';
    if (data.connectionStatus !== "open" && data.pairingCode) {
      document.getElementById("pairing-section").style.display = "block";
      document.getElementById("wa-active").style.display = "none";
      ...
      document.getElementById("wa-active").style.display = data.connectionStatus === "open" ? "block" : "none";
    }

    if (data.cron) {
      document.getElementById("cron").innerHTML = Object.keys(AGENT_META).map(name => {
    ...
    if (data.assignments) {
      document.getElementById("timeline").innerHTML = data.assignments.map(a => {
        const display = a.agent.split("-v")[0];
        const color = (AGENT_META[display] ? AGENT_META[display].color : "#fff");
        return '<div class="tm-entry"><span class="tm-agent" style="color:' + color + '">' + display + '</span><span style="flex:1; margin:0 10px; color:var(--muted)">' + a.task.substring(0,20) + '...</span><b class="badge">' + a.state.toUpperCase() + '</b></div>';
    ...
    if (data.actors) {
      document.getElementById("actors").innerHTML = data.actors.map(a => {
        const color = (AGENT_META[a.displayName] ? AGENT_META[a.displayName].color : "#fff");
        return '<div class="stat-box" style="border-left: 4px solid ' + color + '">' +
               ...
               '<button class="btn" onclick="trigger(\\'' + a.displayName + '\\')">Pulse</button></div></div>';
      }).join('');
    }

    if (data.audits) {
      document.getElementById("audit-table").querySelector("tbody").innerHTML = data.audits.map(a => {
        const display = a.agent.split("-v")[0];
        const color = (AGENT_META[display] ? AGENT_META[display].color : "#fff");
        return '<tr><td>' + a.timestamp + '</td><td style="color:' + color + '; font-weight:800;">' + display + '</td><td style="color:var(--cyan)">' + a.task + '</td><td style="white-space:pre-wrap; color:#d1d5db;">' + a.result + '</td></tr>';
      }).join('');
    }
