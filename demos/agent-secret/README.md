...
```bash
# Create an atespace for the demo actor.
kubectl ate create atespace default

# Create the actor in that atespace.
kubectl ate create actor my-agent -a default --template ate-demo-secret-agent-v2/agent-secret

# Send a request via the Substrate Router (note the atespace in the DNS name).
curl -H "Host: my-agent.default.actors.resources.substrate.ate.dev" http://localhost:8000
```

**What to observe:**
*   In a separate terminal, run `watch kubectl ate get actors -a default`.
...
```bash
for i in {001..023}; do
  kubectl ate create actor session-$i -a default --template ate-demo-secret-agent-v2/agent-secret
done
```
...
for wave in 0 1 2; do
  ...
  for i in {1..8}; do
    ...
    curl -s -H "Host: session-$num.default.actors.resources.substrate.ate.dev" http://localhost:8000 &
  done
