...
// testPodIdentityOID spells out the intended PodIdentity extension OID
// to prevent accidental modification to the value.
var testPodIdentityOID = asn1.ObjectIdentifier{1, 3, 6, 1, 4, 1, 11129, 2, 12, 1}
...
func TestExtensionValueIsJSON(t *testing.T) {
	...
	for _, ext := range cert.Extensions {
		if ext.Id.Equal(testPodIdentityOID) {
			if !json.Valid(ext.Value) {
				t.Errorf("extension value is not valid JSON: %q", ext.Value)
			}
			return
		}
	}
	t.Fatal("PodIdentity extension not found in certificate")
}
