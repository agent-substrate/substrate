...
func TestSnapshotCacheDirSharesActorsMount(t *testing.T) {
	...
	for name, dir := range map[string]string{"SnapshotCacheDir": SnapshotCacheDir, "ActorsDir": ActorsDir} {
		if !strings.HasPrefix(dir, BasePath+"/") {
			t.Errorf("%s = %q, want it under %q so snapshot cache hard links can reach restore dirs", name, dir, BasePath)
		}
	}
}
