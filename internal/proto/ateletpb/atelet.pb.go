...
const (
	...
	VolumeType_VOLUME_TYPE_WORKER_POOL_STORAGE VolumeType = 2
)
...
var (
	VolumeType_name = map[int32]string{
		...
		2: "VOLUME_TYPE_WORKER_POOL_STORAGE",
	}
	VolumeType_value = map[string]int32{
		"VOLUME_TYPE_UNSPECIFIED":         0,
		"VOLUME_TYPE_DURABLE_DIR":         1,
		"VOLUME_TYPE_WORKER_POOL_STORAGE": 2,
	}
)
...
// WorkerPoolStorageVolume references a storage volume declared in the
// WorkerPool's storageVolumes. The named volume is mounted into the ateom
// container by the WorkerPool controller; atelet bind-mounts it (optionally a
// sub-path of it) into the actor's containers.
type WorkerPoolStorageVolume struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// name of the WorkerPool storageVolume to mount.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WorkerPoolStorageVolume) Reset() {
	*x = WorkerPoolStorageVolume{}
	mi := &file_atelet_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WorkerPoolStorageVolume) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkerPoolStorageVolume) ProtoMessage() {}

func (x *WorkerPoolStorageVolume) ProtoReflect() protoreflect.Message {
	mi := &file_atelet_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use WorkerPoolStorageVolume.ProtoReflect.Descriptor instead.
func (*WorkerPoolStorageVolume) Descriptor() ([]byte, []int) {
	return file_atelet_proto_rawDescGZIP(), []int{6}
}

func (x *WorkerPoolStorageVolume) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
...
func (x *Volume) GetWorkerPoolStorage() *WorkerPoolStorageVolume {
	if x != nil {
		if x, ok := x.Source.(*Volume_WorkerPoolStorage); ok {
			return x.WorkerPoolStorage
		}
	}
	return nil
}
...
type Volume_WorkerPoolStorage struct {
	WorkerPoolStorage *WorkerPoolStorageVolume `protobuf:"bytes,4,opt,name=worker_pool_storage,json=workerPoolStorage,proto3,oneof"`
}

func (*Volume_DurableDir) isVolume_Source() {}

func (*Volume_WorkerPoolStorage) isVolume_Source() {}

type VolumeMount struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	Name      string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	MountPath string                 `protobuf:"bytes,2,opt,name=mount_path,json=mountPath,proto3" json:"mount_path,omitempty"`
	// read_only mounts the volume read-only inside the container.
	ReadOnly bool `protobuf:"varint,3,opt,name=read_only,json=readOnly,proto3" json:"read_only,omitempty"`
	// sub_path mounts a sub-directory of the volume instead of its root. The
	// literal "${ACTOR_ID}" is replaced with the actor's ID at mount time,
	// enabling per-actor subdirectories on shared storage.
...
const file_atelet_proto_rawDesc = "" +
	...
	"\x10DurableDirVolume\"-\n" +
	"\x17WorkerPoolStorageVolume\x12\x12\n" +
	...
	"durableDir\x12Q\n" +
	"\x13worker_pool_storage\x18\x04 \x01(\v2\x1f.atelet.WorkerPoolStorageVolumeH\x00R\x11workerPoolStorageB\b\n" +
	"\x06source\"x\n" +
	...
	"\bsub_path\x18\x04 \x01(\tR\asubPath\"\xd5\x01\n" +
	"\tContainer\x12\x12\n" +
	...
	"\x17VOLUME_TYPE_DURABLE_DIR\x10\x01\x12#\n" +
	"\x1fVOLUME_TYPE_WORKER_POOL_STORAGE\x10\x02*j\n" +
...
var file_atelet_proto_goTypes = []any{
	...
	(*WorkerPoolStorageVolume)(nil),         // 9: atelet.WorkerPoolStorageVolume
...
var file_atelet_proto_depIdxs = []int32{
	...
	9,  // 8: atelet.Volume.worker_pool_storage:type_name -> atelet.WorkerPoolStorageVolume
