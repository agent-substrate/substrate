...
// AteomForActorSPIFFEID returns "spiffe://substrate-actor.local/atespace/<atespace>/actor/<name>",
...
// ActorRefFromAteomForActorSPIFFEID parses an ID built by ActorSPIFFEID.
...
func ActorRefFromAteomForActorSPIFFEURL(u *url.URL) (ActorRef, error) {
	if u.Scheme != "spiffe" || u.Host != ActorSPIFFETrustDomain || u.User != nil || u.RawQuery != "" || u.Fragment != "" {
		return ActorRef{}, fmt.Errorf("ateom-for-actor SPIFFE ID must have format spiffe://<trust.domain>/ateom-for-actor/<atespace>/<name>")
	}
