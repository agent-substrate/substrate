...
func testActorDirs() *ateompb.ActorDirs {
	return &ateompb.ActorDirs{
		...
		RunscStateDir:             "/node/actors/a/runsc-state",
		PidFileDir:                "/node/actors/a/pidfiles",
...
func TestValidateActorDirs(t *testing.T) {
	for _, tc := range []struct {
		...
		{"outside root", func(actorDirs *ateompb.ActorDirs) *ateompb.ActorDirs {
			actorDirs.VolumesDir = "/node/actors/b/volumes"
			return actorDirs
		}, "actor_dirs.volumes_dir"},
		{"root itself", func(actorDirs *ateompb.ActorDirs) *ateompb.ActorDirs {
			actorDirs.PidFileDir = actorDirs.RootDir
			return actorDirs
		}, "actor_dirs.pid_file_dir"},
