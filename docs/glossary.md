...
- **Snapshot scope**: what an `ActorTemplate`'s `SnapshotsConfig` includes
  ...
  - **`Data`**: only the contents of attached volumes that support
    ...
    [Data resume policy](#snapshots): cold-boot by default, or combined
    with the [Golden Snapshot](#snapshots).
...
- **Data resume policy**: an `ActorTemplate`'s `dataResumePolicy` selects how
  an Actor boots when a Resume can only use the data part of a snapshot (a
  `Data`-scope snapshot from either trigger):
  - **`ColdBoot`** (default): start the containers afresh from the OCI image
    with the `DurableDir` contents restored.
  - **`OnGolden`**: restore the template's [Golden Snapshot](#snapshots)
  ...
  A still-valid `Full` snapshot always restores from its own content and
  ignores the policy.
...
- **Golden Snapshot**: the initial checkpoint captured once, when an
  ...
  snapshot. It is always a `Full` capture, and under the `OnGolden`
  [Data resume policy](#snapshots) it also supplies the guest state that an
  Actor's `Data` snapshot is combined with on Resume.
