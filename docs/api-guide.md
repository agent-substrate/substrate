...
atelet resolves the bundle on the node when the actor starts, reading the backing object through a cluster-wide watch (the same informer that drives live refresh) and sanitizing it the way kubelet does for projections: only `CERTIFICATE` PEM blocks are kept, deduplicated, with block headers stripped and the anchors deliberately shuffled — order carries no meaning, so consumers must not depend on it. The actor itself never talks to any bundle backend. Starting the actor fails, with an error naming the bundle, if the name is not on the allowlist, the bundle's backend is unavailable in this deployment, or the resolved bundle is missing, empty, or contains no certificates.

Bundle contents are re-resolved on every Run/Restore, and refreshed live while the actor runs: when the backing bundle changes, atelet rewrites the projected file in place — atomically, at the same path. The reader contract matches the Kubernetes clusterTrustBundle projection's: the platform rewrites the file and the application re-reads it to pick up rotations, so a runtime that loads trust anchors once at process start observes updates only at its next start or resume. A refresh that fails (the backing object deleted or unusable) leaves the last good contents in place — fail-closed applies at actor start only, and a running actor never sees the file go missing or empty. Correctness therefore never rests on propagation latency: bundle publishers rotate with overlap, adding a new CA before any leaf is minted under it and keeping the old CA until its last leaf expires.
