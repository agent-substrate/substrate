...
| `defaultEgressPolicy` | `EgressPolicyTemplate` | Optional. A `rules` list with the same validation as `EgressPolicy`, without resource metadata. Actor creation atomically copies it into the actor's `default` policy, with the actor's atespace and fresh server-generated metadata. An empty block creates a deny-all policy; omitting it creates no policy. Callers can update or delete the actor's policy independently; it does not track later template changes. |
