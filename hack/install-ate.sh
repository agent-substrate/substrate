...
usage() {
  ...
  echo "  ATE_API_POSTGRES_OWNER_CONNECTION_STRING       Optional owner DSN; defaults to the legacy DSN, then read/write DSN"
  echo "  ATE_API_POSTGRES_CONNECTION_STRING     Legacy DSN for both pools; remains the owner DSN if read/write is set separately"
