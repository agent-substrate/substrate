...
cat <<EOF
  ...
  2. Create an atespace, then create + resume an actor (kubectl-ate; install with: go install ./cmd/kubectl-ate):
       kubectl ate${KCTX_FLAG} create atespace default
       kubectl ate${KCTX_FLAG} create actor my-counter-1 -a default \\
  ...
  3. Port-forward the atenet-router and curl the in-RAM counter:
     ...
     Increment, suspend (kubectl ate suspend actor my-counter-1 -a default), resume on another
